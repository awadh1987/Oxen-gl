...
/**
 * Cache vehicle lookup by IMEI to minimize DB roundtrips during streaming
 */
...
/**
 * Parse AVL Data Packet (Codec 8 and Codec 8 Extended)
 */
function parseAvlPacket(buffer, offset = 0) {
  let cur = offset;
  if (buffer.length < cur + 10) return null;

  // Check optional 4-byte 0x00000000 preamble
  ...
  for (let i = 0; i < recordsCount; i++) {
    if (codecId === 0x08) {
      ...
      // Codec 8 Extended 2-byte IO structure
      cur += 2; // Event IO ID
      cur += 2; // Total IO
  ...
  socket.on('data', async (chunk) => {
    ...
    try {
      if (state === 'WAITING_IMEI') {
        // Handshake: receive IMEI
        // Format A: 2 bytes length (0x00 0x0F) + 15 ASCII bytes
        // Format B: 15 ASCII bytes direct
        let parsedImei = null;
        let consumed = 0;
      ...
      if (state === 'WAITING_AVL' && buffer.length >= 12) {
        ...
        if (avlResult) {
          ...
          // Ensure vehicle resolution has resolved
          ...
          // Acknowledge records count (4-byte integer)
...
export { server, parseAvlPacket, getOrCreateVehicle };
