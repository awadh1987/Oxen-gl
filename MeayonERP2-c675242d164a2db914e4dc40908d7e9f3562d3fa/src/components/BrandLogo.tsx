...
export const BrandLogo: React.FC<BrandLogoProps> = ({
  ...
  const effectiveLogoUrl = propCustomLogoUrl ?? currentCompany?.uiLogoUrl ?? appBrandConfig?.customLogoUrl;
  const effectiveNameAr = propCompanyNameAr ?? currentCompany?.name ?? appBrandConfig?.companyNameAr ?? 'ميون مقاولات ولوجستيات';
  const effectiveNameEn = propCompanyNameEn ?? currentCompany?.name ?? appBrandConfig?.companyNameEn ?? 'MYON CONTRACTING & LOGISTICS';
