...
export const LandingPageView: React.FC<LandingPageViewProps> = ({ onLoginSuccess }) => {
  ...
  const handleQuickRoleSelect = (role: UserRole) => {
    ...
    const matchedUser = role === 'Admin'
      ? users.find((u) => u.email === 'admin@meayon.local') || users.find((u) => u.role === role)
      : users.find((u) => u.role === role);
    if (matchedUser) {
      setSelectedUserId(matchedUser.id);
      setUsernameInput(matchedUser.username);
      setPasswordInput(role === 'Admin' ? 'ChangeMe123!' : '');
    }
  };
  ...
  const handleSignUpSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!signupForm.fullName || !signupForm.username || !signupForm.email || registrationPassword.length < 8) return;
    ...
    if (!response.ok) {
      ...
      alert(error.detail || (isAr ? 'تعذر إرسال طلب التسجيل.' : 'Could not submit registration request.'));
      return;
    }
    ...
    const persistedRequest = {
      ...newReq,
      id: savedRequest.id,
      requestedAt: savedRequest.requestedAt || newReq.requestedAt,
    };
    const current = JSON.parse(localStorage.getItem('meayon_user_requests') || '[]');
    localStorage.setItem('meayon_user_requests', JSON.stringify([persistedRequest, ...current]));
            ...
            <div className="space-y-6 text-center lg:col-span-7 lg:text-start">
              ...
              <p className="max-w-2xl text-sm text-neutral-300 sm:text-base leading-relaxed">
                {isAr
                  ...
                  : brandConfig.homepageHeroSubtitleEn ||
                    'Meayon Economic Contracting Co. Ltd. manages a complete end-to-end operational lifecycle for building materials haulage, crusher payables, weighbridge scale tickets, and ZATCA Phase-2 certified invoicing.'}
              </p>
              ...
              <div className="grid grid-cols-2 gap-4 border-t border-neutral-800/80 pt-6 sm:grid-cols-4">
                ...
                <div>
                  ...
                  <div className="text-[11px] text-neutral-400">{isAr ? 'كسارة ومحجر معتمد' : 'Partner Crushers'}</div>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-5">
              <div className="relative overflow-hidden rounded-3xl border border-neutral-800 bg-neutral-900/90 p-6 shadow-2xl backdrop-blur-xl">
                ...
                <div className="mt-4 space-y-3">
                  ...
                  {/* Native login shortcuts */}
                  <div className="rounded-2xl border border-neutral-800 bg-neutral-950/80 p-4">
                    <div className="flex items-center justify-between mb-2.5">
                      <p className="text-xs font-bold text-neutral-300">
                        {isAr ? 'الدخول الآمن حسب الدور:' : 'Secure role-based access:'}
                      </p>
                      ...
                      >
                        <div>
                          ...
                          <div className="text-[9px] text-emerald-400">
                            {isAr ? 'المدير المالي • مراجعة الفواتير والكسارات' : 'Finance Director • Accounting'}
                          </div>
                        </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center space-y-3 max-w-3xl mx-auto">
            ...
            <p className="text-xs sm:text-sm text-slate-400">
              {isAr
                ? 'حلول متكاملة تضمن دقة الأوزان وسرعة الإمداد والشفافية المالية التامة بين الكسارات ومقاولي النقل والعملاء.'
          ...
          <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="rounded-3xl border border-slate-800 bg-slate-950/80 p-6 transition-all hover:border-amber-500/50">
              ...
              <h3 className="mt-4 text-sm font-black text-white">
                {isAr ? 'عقود توريد وتغطية الكسارات' : 'Crusher & Quarry Networks'}
              </h3>
              <p className="mt-2 text-xs text-slate-400 leading-relaxed">
                {isAr
            ...
            <div className="rounded-3xl border border-slate-800 bg-slate-950/80 p-6 transition-all hover:border-emerald-500/50">
              ...
              <p className="mt-2 text-xs text-slate-400 leading-relaxed">
                {isAr
                  ? 'حساب تلقائي للفارق بين الوزن المحمل والوزن الصافي المستلم لكل رحلة وتحديد نسب الهدر لكل ناقل.'
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="rounded-3xl border border-orange-900/30 bg-gradient-to-br from-neutral-900/90 via-[#181818] to-[#121212] p-8 sm:p-12 lg:flex lg:items-center lg:justify-between gap-8">
              ...
              <p className="text-xs sm:text-sm text-neutral-300 leading-relaxed">
                {isAr
                  ...
                  : brandConfig.homepageAboutDescriptionEn ||
                    'Meayon Economic Contracting Co. Ltd. enforces enterprise-grade audit logging, digital cryptographic approvals, soft-delete recoverability, and multi-tier role access.'}
              </p>
            </div>
            ...
            <div className="mt-8 lg:mt-0 flex flex-col gap-3 shrink-0">
              ...
              >
                <UserPlus className="h-4 w-4 text-[#F05627]" />
                <span>{isAr ? 'طلب اعتماد مورد أو ناقل جديد' : 'Register New Subcontractor'}</span>
              </button>
            </div>
          </div>
        </div>
              ...
              {authTab === 'signin' ? (
                ...
                <div className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full rounded-xl border border-neutral-800 bg-neutral-900 py-2.5 text-xs font-bold text-neutral-400 hover:bg-neutral-800"
                  >
                    {isAr ? 'إلغاء' : 'Cancel'}
                  </button>
                </div>
