...
export const AIOperationsView: React.FC = () => {
  ...
  const [mapsQuery, setMapsQuery] = useState(
    isAr ? 'أقرب كسارات رمل وحصى ومحطات ميزان شاحنات حول الرياض' : 'Quarries, crushers and truck weighbridges near Riyadh'
  );
  ...
  const [imagePrompt, setImagePrompt] = useState(
    isAr
      ? 'ختم أمني رقمي معتمد لميزان البسكول لشركة ميون للمقاولات باللون الأخضر والذهبي مع باركود'
      : 'Professional circular security approved stamp badge for Meayon Contracting weighbridge scale ticket'
  );
  ...
  const quickScenarios = [
    {
      ...
      query:
        'قم بفحص سجلات العمليات وحدد الشاحنات أو الناقلين الذين تتجاوز نسبة الفاقد لديهم 1.5% مع تحديد الأثر المالي والاشتباه في سرقة المواد أو تلاعب الميزان.',
    },
    {
      titleAr: 'إعداد تقرير الإدارة التنفيذية الشهري',
      titleEn: 'Generate Executive Monthly Operations Brief',
      query:
        'قم بصياغة ملخص تنفيذي رفيع المستوى لمجلس إدارة شركة ميون يلخص حجم المبيعات، أداء الكسارات، ومؤشرات الربحية.',
    },
    {
      titleAr: 'تحليل كفاءة الكسارات وهوامش الربح',
      titleEn: 'Crusher Pricing & Margin Optimization',
      query:
        'قارن بين أسعار الشراء من مختلف الكسارات والعملاء المستلمين لتحديد أفضل مسارات النقل ربحية وأعلى الكسارات موثوقية.',
    },
  ];
  ...
  const handleRunAnalysis = async (customQuery?: string) => {
    ...
    try {
      ...
      setTimeout(() => {
        setAnalysisResult({
          executiveSummaryAr:
            'بناءً على تدقيق 40 رحلة توريد لشهر أغسطس 2026، حققت شركة ميون إجمالي مبيعات بلغ 88,400 ر.س بإجمالي حمولة 1,940 طن. يبلغ متوسط هامش الربح التشغيلي 41.5% وهو مؤشر ممتاز.',
          executiveSummaryEn:
            'Audit completed across 40 haulage trips for August 2026. Meayon achieved SAR 88,400 sales with 1,940 metric tons delivered and 41.5% gross profit margin.',
          ...
          crusherFinancialAnalysis:
            'كسارة اليمامة تمثل 45% من حجم التوريد بأسعار تنافسية (28 ر.س للطن). كسارة طوق تسجل أعلى هامش ربح للمتر المكعب.',
          ...
          anomaliesDetected: [
            {
              ...
              severity: 'Medium',
              details:
                'سجلت الشاحنة 3190 فاقداً بمقدار 1.4 طن في رحلة واحدة من كسارة طوق إلى يوني بيتون.',
              actionPlan: 'خصم قيمة الفاقد من مستحقات الناقل ومعايرة ميزان البسكول.',
            },
            {
              title: 'تركز التوريد في كسارة واحدة',
              severity: 'Low',
              details: '45% من التوريدات تأتي من كسارة اليمامة.',
...
- Diesel prices follow periodic revisions from Saudi Aramco.`,
  ...
  // 6. Gemini 3.1 Flash Image Generation / Editing
  ...
  return (
    <div className="space-y-6" id="ai-operations-view">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                ...
                <span>{isAr ? 'خرائط جوجل ومواقع الكسارات ومحطات الميزان الذكية' : 'Google Maps Geospatial Quarry & Scale Grounding'}</span>
              </h3>
              <span className="rounded-full bg-emerald-50 px-2.5 py-0.5 text-[10px] font-bold text-emerald-700">
                Gemini 3.5 Flash + Maps Tool
              </span>
            </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                ...
                <span>{isAr ? 'البحث الحي في اللوائح والأنظمة (Google Search Grounding)' : 'Live Regulatory & Market Grounding (Google Search)'}</span>
              </h3>
              <span className="rounded-full bg-blue-50 px-2.5 py-0.5 text-[10px] font-bold text-blue-700">
                Gemini 3.5 Flash + Search Tool
              </span>
            </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                ...
                <span>{isAr ? 'استوديو توليد الفيديو بالذكاء الاصطناعي (Google Veo)' : 'Google Veo Cinematic Video Studio'}</span>
              </h3>
      ...
      {/* TAB 6: Image Studio (Create & Edit with Gemini 3.1 Flash Image) */}
      {activeTab === 'image-studio' && (
        <div className="space-y-6">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                ...
                <span>{isAr ? 'استوديو الصور والأختام الرقمية (Gemini 3.1 Flash Image)' : 'AI Image Creation & Editing Studio'}</span>
              </h3>
            ...
            <div className="space-y-4 mb-4">
              ...
              >
                {imageLoading ? (
                  <>
                    <RefreshCw className="h-4 w-4 animate-spin" />
                    <span>{isAr ? 'جاري المعالجة والتوليد...' : 'Processing with Gemini...'}</span>
                  </>
                ) : (
                  <>
                    <Sparkles className="h-4 w-4" />
