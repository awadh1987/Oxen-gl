...
هاتف: ${brandConfig.phone} | ${brandConfig.email}`;
  ...
  // Direct native system print
  const handlePrintDirectly = () => {
    // Inject dynamic print styling for invoice
    ...
    >
      ...
      >
        ...
        <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
          ...
          <div className="space-y-3 pt-2">
            <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider">
              {isAr ? '٣. إجراءات الإرسال والطباعة والتنزيل:' : '3. Dispatch, Print & Download Actions:'}
            </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-teal-600 to-emerald-600 px-4 py-3.5 text-xs font-black text-white shadow-md shadow-teal-600/30 hover:opacity-95 transition"
              >
                <Printer className="h-4 w-4" />
                <span>{isAr ? 'طباعة مباشرة (Print)' : 'Print Directly'}</span>
              </button>

              {/* Action 2: Direct File Download */}
              <button
        ...
        <div
          ...
          className="absolute left-0 top-0 pointer-events-none w-[900px] -z-10"
          aria-hidden="true"
        >
