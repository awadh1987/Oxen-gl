...
export const BrandLogo: React.FC<BrandLogoProps> = ({
  ...
  let activeTenantLicense: any = null;
  try {
    const appContext = useApp();
    appBrandConfig = appContext?.brandConfig;
    activeTenantId = appContext?.activeTenantId;
    activeTenantLicense = appContext?.activeTenantLicense;
  } catch {
  ...
  const defaultPlatformNameAr = localStorage.getItem('oxengl_platform_name_ar') || 'منصة OxenGL العالمية المركزية';
  const defaultPlatformNameEn = localStorage.getItem('oxengl_platform_name_en') || 'OXENGL GLOBAL CLOUD PLATFORM';

  const effectiveLogoUrl = propCustomLogoUrl ?? (isPlatformMaster ? (localStorage.getItem('oxengl_platform_logo_url') || '/logo.png') : (appBrandConfig?.customLogoUrl ?? '/logo.png'));
  const effectiveNameAr = propCompanyNameAr ?? (isPlatformMaster ? defaultPlatformNameAr : (activeTenantLicense?.companyName || appBrandConfig?.companyNameAr || 'شركة ميون الاقتصادية المحدودة'));
  const effectiveNameEn = propCompanyNameEn ?? (isPlatformMaster ? defaultPlatformNameEn : (activeTenantLicense?.companyNameEn || appBrandConfig?.companyNameEn || 'MAYON ECONOMIC COMPANY LTD'));
