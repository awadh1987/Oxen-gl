...
export const ExportPrintModal: React.FC<ExportPrintModalProps> = ({
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      const pngDataUrl = await captureElementToPng(printPreviewRef.current, 1.5);
  ...
  const handleSaveToDrive = async () => {
    ...
    try {
      ...
      if (!isDriveAuthenticated()) {
        ...
        if (!ok) {
          alert(isAr ? 'يرجى تسجيل الدخول إلى Google Drive أولاً.' : 'Please connect Google Drive first.');
      ...
      const pdfBytes = await pdfDoc.save();
      const blob = new Blob([pdfBytes], { type: 'application/pdf' });
      const fileName = `Meayon_${docType}_${docRefNumber}.pdf`;

      const uploadResult = await uploadFileToDrive(
        blob,
        fileName,
        'application/pdf',
        `نسخة أصلية محفوظة من منظومة ميون ERP (${docType}) رقم ${docRefNumber}`
      );

      if (uploadResult.success) {
        setDriveSaveSuccess(true);
        setTimeout(() => setDriveSaveSuccess(false), 3500);
      } else {
        alert(uploadResult.error || (isAr ? 'تعذر حفظ الملف في Google Drive.' : 'Could not save to Google Drive.'));
      }
    } catch (err: any) {
      console.error('Google Drive Export Error:', err);
      alert(err.message || (isAr ? 'حدث خطأ أثناء الرفع إلى Google Drive.' : 'Error saving to Google Drive.'));
    } finally {
      setIsSavingToDrive(false);
    }
  };
      ...
      <div className="flex h-[96vh] w-full max-w-[1440px] flex-col rounded-3xl border border-slate-700/80 bg-slate-900 shadow-2xl overflow-hidden text-slate-100">
        ...
        <div className="flex flex-wrap items-center justify-between border-b border-slate-800 bg-slate-900/90 px-5 py-3.5 gap-3 shrink-0">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              id="export-modal-save-drive-btn"
              onClick={handleSaveToDrive}
              disabled={isSavingToDrive}
              className="flex items-center gap-1.5 rounded-xl border border-indigo-500/40 bg-indigo-950/60 px-3.5 py-2 text-xs font-bold text-indigo-200 hover:bg-indigo-900 transition-colors disabled:opacity-50 cursor-pointer"
              title={isAr ? 'حفظ نسخة PDF في Google Drive' : 'Save PDF copy to Google Drive'}
            >
              ...
              <span>
                {isSavingToDrive
                  ...
                  : driveSaveSuccess
                  ? (isAr ? 'تم الحفظ في Drive!' : 'Saved to Drive!')
                  : (isAr ? 'حفظ في Drive' : 'Save to Drive')}
              </span>
            </button>
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto bg-slate-950/60 p-4 sm:p-8 flex justify-center items-start">
            <div
              ...
              className={`export-preview-frame w-full ${orientation === 'landscape' ? 'export-preview-landscape' : 'export-preview-portrait'}`}
            >
              ...
              <div
                ...
                ref={printPreviewRef}
                dir={isAr ? 'rtl' : 'ltr'}
                className={`printable-document export-preview-paper mx-auto bg-white text-slate-900 shadow-2xl rounded-2xl border border-slate-200 p-6 sm:p-10 font-sans transition-all ${
                  orientation === 'landscape' ? 'export-paper-landscape' : 'export-paper-portrait'
                }`}
              >
                {/* 1. Official Header */}
                {showLetterhead && (
                    ...
                    <table className="w-full text-right text-xs border-collapse">
                      <thead>
                        <tr className="bg-slate-900 text-white font-bold">
                          ...
                          <th className="p-3 text-center">{isAr ? 'الوزن (طن)' : 'Qty (Tons)'}</th>
                    ...
                    <table className="w-full text-right text-[11px] border-collapse">
                      <thead>
                        <tr className="bg-slate-900 text-white font-bold">
                          ...
                          <th className="p-2 text-center">{isAr ? 'محمل (طن)' : 'Loaded'}</th>
                          <th className="p-2 text-center">{isAr ? 'مستلم (طن)' : 'Delivered'}</th>
                          <th className="p-2 text-center">{isAr ? 'الفاقد' : 'Loss'}</th>
                      ...
                      <table className="w-full text-right text-xs border-collapse">
                        <thead>
                          <tr className="bg-slate-900 text-white font-bold">
                            ...
                            <th className="p-2.5 text-center">{isAr ? 'الوزن (طن)' : 'Weight'}</th>
