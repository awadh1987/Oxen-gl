      ...
      >
        ...
        <div className="mt-4 rounded-xl border border-slate-300 bg-slate-50/50 p-5 font-mono text-xs text-slate-800">
          ...
          <div className="flex items-center justify-between border-b border-slate-300 pb-3 text-center">
            <BrandLogo size="sm" showText={false} customLogoUrl={brandConfig.customLogoUrl} companyNameAr={brandConfig.companyNameAr} />
            <div>
              ...
              <p className="text-[9px] text-blue-800 font-sans mt-0.5">
                تذكرة وزن ميزان شاحنات معتمد (Weighbridge Scale Ticket)
              </p>
            </div>
            <div className="text-right text-[10px] text-slate-500">
              <p>رقم السجل: {brandConfig.crNumber}</p>
              <p>الرقم الضريبي: {brandConfig.taxNumber}</p>
            </div>
          </div>
          ...
          <div className="my-3 grid grid-cols-2 gap-2 border-b border-slate-200 pb-3 text-[11px]">
            ...
            <div>
              <span className="text-slate-500">{isAr ? 'الناقل / السائق:' : 'Transporter:'}</span>{' '}
              <span className="font-bold">{operation.transporter_name}</span>
            </div>
            <div>
              <span className="text-slate-500">{isAr ? 'مصدر التحميل (الكسارة):' : 'Quarry Source:'}</span>{' '}
