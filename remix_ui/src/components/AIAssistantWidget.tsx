...
أنا **المساعد الذكي لعمليات شركة ميون للمقاولات المحدودة**، مدعوم بنماذج **Gemini AI** المتطورة.
...
- 🏭 **مطابقة حسابات الكسارات** وأرصدة الموردين وسندات الصرف ومقارنة أسعار الشراء.
- 🧾 **مراجعة فواتير العملاء** والامتثال لضريبة القيمة المضافة ZATCA (15%).
- 🌐 **البحث المباشر (Google Search)** للوائح النقل وأسعار الديزل الرسمية.
- 📍 **الخرائط اللوجستية (Google Maps)** لمواقع الكسارات ومحطات الميزان الذكية.

كيف يمكنني مساعدتك اليوم؟`
          : `Hello **${currentUser.fullName}**! 👋

I am your **AI Logistics & Operational Auditor** for **Meayon Contracting Co. Ltd.** powered by **Gemini AI Suite**.

I have real-time visibility into your fleet operations, weighbridge tickets, crusher accounts, customer billings, plus live Google Search & Maps Grounding. 
        ...
        suggestedActions: [
          {
            ...
            query: 'أريد تقريراً تفصيلياً بالرحلات التي تجاوز فيها الفاقد 1.5 طن والناقلين المسؤولين عنها.',
          },
          {
            labelAr: '📊 ملخص الأرباح ومستحقات الكسارات',
            labelEn: '📊 Profit Margins & Crusher Payables',
            query: 'ما هو صافي أرباح العمليات الحالية وإجمالي مستحقات الكسارات المتبقية؟',
          },
          {
            labelAr: '📍 مواقع الكسارات ومحطات الميزان بالرياض',
            labelEn: '📍 Quarry Locations & Weigh Stations',
            query: 'ما هي أقرب محطات ميزان وكسارات رمل معتمدة حول جنوب وشرق الرياض؟',
          },
        ],
      },
    ];
...
- **Alert**: Recommend inspecting scale tickets for Truck 3190 (Fast Transport) due to 1.4 tons loss to UniBeton.`,
  ...
  const quickChips = [
    {
      ...
      query: 'حلل سجلات الفاقد وحدد الشاحنات والناقلين الذين تجاوزوا الحد المسموح به مع حساب الأثر المالي.',
    },
    {
      icon: Building2,
      labelAr: 'مطابقة الكسارات والمستحقات',
      labelEn: 'Crusher Balances',
      query: 'ما هي مستحقات الكسارات غير المسددة ومقارنة أسعار الشراء للطن بين مختلف الكسارات؟',
    },
    {
      icon: MapPin,
      labelAr: 'مواقع الكسارات والموازين',
  ...
  return (
    <>
      {/* Floating Smart Launcher Button (When widget is minimized) - Compact & Elegant */}
      {!isOpen && (
        <motion.div
          initial={{ scale: 0, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          transition={{ type: 'spring', stiffness: 260, damping: 20 }}
          className={`fixed bottom-4 sm:bottom-5 ${isAr ? 'left-4 sm:left-5' : 'right-4 sm:right-5'} z-40`}
        >
          <button
            id="floating-ai-assistant-button"
            onClick={() => setIsOpen(true)}
            className="group relative flex items-center gap-2 rounded-full border border-orange-400/40 bg-gradient-to-r from-slate-900/95 via-slate-900/90 to-neutral-900/95 px-3 py-2 text-white shadow-lg shadow-black/30 backdrop-blur-md transition-all hover:border-orange-400 hover:scale-105 active:scale-95 cursor-pointer ring-1 ring-white/10"
            title={isAr ? 'المساعد الذكي (Gemini AI) - اختصار Ctrl + J' : 'AI Operations Auditor (Ctrl + J)'}
          >
            {/* Small Glowing Accent */}
            <div className="relative flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-gradient-to-tr from-orange-600 to-amber-500 text-white shadow-xs">
            ...
            <div className="flex items-center gap-1.5">
              <span className="text-xs font-bold tracking-tight text-white group-hover:text-orange-300 transition-colors">
                {isAr ? 'المساعد الذكي' : 'Gemini AI'}
              </span>
              <span className="rounded-md bg-orange-500/20 px-1.5 py-0.5 text-[9px] font-mono font-bold text-orange-300 border border-orange-400/30 hidden sm:inline-block">
                AI
              </span>
            </div>
          </button>
                  ...
                  <div>
                    <div className="flex items-center gap-1.5">
                      <h2 className="text-xs font-black tracking-wide">
                        {isAr ? 'مساعد ميون الذكي' : 'Meayon AI Assistant'}
                      </h2>
              ...
              <div className="flex shrink-0 items-center justify-between border-b border-slate-100 bg-slate-50/95 px-3 py-1.5 text-[11px]">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    title={isAr ? 'تفعيل البحث المباشر في الويب' : 'Google Search Grounding'}
                  >
                  ...
                  <button
                    ...
                    title={isAr ? 'تفعيل خرائط جوجل للمواقع والموازين' : 'Google Maps Grounding'}
                  >
