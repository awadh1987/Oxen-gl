...
export const PrintableSupportingTicket: React.FC<PrintableSupportingTicketProps> = ({
  ...
  return (
    ...
    >
      {/* Official Header with Meayon Logo and Registration info */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 my-5 text-xs">
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 space-y-2.5">
          <div className="flex items-center gap-2 border-b border-slate-200 pb-2 text-slate-900 font-bold">
            ...
            <span>{isAr ? 'بيانات الشاحنة والناقل' : 'Truck & Carrier Details'}</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-[11px]">
            ...
            <div>
              <span className="text-slate-500 block">{isAr ? 'اسم الناقل / السائق:' : 'Transporter:'}</span>
        ...
        {/* Box 2: Source Crusher & Destination Customer */}
        <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 space-y-2.5">
          <div className="flex items-center gap-2 border-b border-slate-200 pb-2 text-slate-900 font-bold">
            <MapPin className="h-4 w-4 text-emerald-600" />
            <span>{isAr ? 'مسار التوريد وموقع الكسارة' : 'Supply Route & Source'}</span>
          </div>

          <div className="grid grid-cols-2 gap-2 text-[11px]">
            <div>
              <span className="text-slate-500 block">{isAr ? 'مصدر التحميل (الكسارة):' : 'Loading Crusher:'}</span>
            ...
            <div>
              <span className="text-slate-500 block">{isAr ? 'رقم فاتورة الكسارة:' : 'Crusher Inv No:'}</span>
      ...
      <div className="my-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 bg-white p-3 rounded-xl border border-slate-200">
          ...
          <div className="space-y-1.5 text-[11px] text-slate-600 flex-1">
            ...
            <p>
              • {isAr ? 'معتمد من نظام إدارة الأسطول الميداني لشركة ميون' : 'Verified by Meayon Fleet Management Gateway'}
            </p>
