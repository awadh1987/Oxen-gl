...
export const CreateVoucherModal: React.FC<CreateVoucherModalProps> = ({
  ...
  const handlePartySelect = (selectedId: string) => {
    ...
    if (partyType === 'Crusher') {
      ...
      if (crush) {
        const crushName = crush.crusherName || (crush as any).name;
        setPartyName(crushName);
        setPartyTaxNumber(crush.taxNumber || '');
        if (!purpose) {
          setPurpose(`سداد مستحقات توريد مواد حصوية لـ (${crushName})`);
        }
      }
    } else if (partyType === 'Transporter') {
      const trans = transporters.find((t) => t.id === selectedId);
      if (trans) {
        const transName = trans.transporterName || (trans as any).name;
        setPartyName(transName);
        setPartyTaxNumber((trans as any).taxNumber || '');
        if (!purpose) {
          setPurpose(`سداد أجور نقل وتوريد لشاحنات أسطول (${transName})`);
        }
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const voucherNumber = `${type === 'Payment' ? 'PV' : 'RV'}-${new Date(date).getFullYear()}-${String(new Date(date).getMonth() + 1).padStart(2, '0')}-${String(Date.now()).slice(-6)}`;
    let persistedId: string | undefined;
    try {
      const persisted = await apiService.createVoucher({
        voucher_type: type,
        voucher_number: voucherNumber,
        beneficiary: partyName.trim(),
        amount: Number(amount),
        status: autoApprove && (isAdmin || isCOO) ? 'Approved' : 'Pending_Approval',
        notes: notes.trim() || purpose.trim() || undefined,
      });
      persistedId = persisted.id;
    } catch (error) {
      alert(error instanceof Error ? error.message : (isAr ? 'تعذر حفظ السند في الخادم.' : 'Could not save voucher on the server.'));
      return;
    }
                    ...
                    {partyType === 'Crusher' &&
                      crushers.map((c) => (
                        <option key={c.id} value={c.id}>
                          {c.crusherName || (c as any).name}
                        </option>
                      ))}
                    {partyType === 'Transporter' &&
                      transporters.map((t) => (
                        <option key={t.id} value={t.id}>
                          {t.transporterName || (t as any).name}
                        </option>
                      ))}
                    {partyType === 'Customer' &&
                      customers.map((c) => (
