...
export const ExecutiveAdminView: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    'approvals' | 'global-grid' | 'branding' | 'saas-licenses' | 'audit-trail' | 'recycle-bin'
  >('approvals');
  ...
  const exportAuditLogsCSV = () => {
    const headers = ['Log ID', 'Timestamp', 'User', 'Role', 'Action', 'Entity Type', 'Entity ID', 'IP Address', 'Summary'];
  ...
  const exportAuditLogsJSON = () => {
    ...
    link.setAttribute('download', `audit_trail_compliance_${new Date().toISOString().slice(0, 10)}.json`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  const copyLogJSON = (log: any) => {
    ...
    <div className="space-y-6" id="executive-admin-view">
      ...
      <div className="flex flex-wrap gap-2 border-b border-slate-200 pb-3">
        {[
          ...
          {
            id: 'saas-licenses',
            labelAr: 'تراخيص الاشتراك والعملاء',
            labelEn: 'SaaS Tenant Licenses',
            icon: KeyRound,
          },
          {
            id: 'audit-trail',
      ...
      {/* ======================================================== */}
      {/* TAB 4: AUDIT TRAIL LOGS / ACTIVITY LOG                   */}
      {/* ======================================================== */}
      {activeTab === 'saas-licenses' && <SaaSLicenseProvisioner />}

      {activeTab === 'audit-trail' && (
        <div className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-xs space-y-6">
          {/* Header & Description */}
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center border-b border-slate-100 pb-4">
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base font-black text-slate-900">
                  {isAr ? 'سجل التدقيق الشامل والأنشطة (Activity Log & Audit Trail)' : 'Executive Audit Trail & Activity Log'}
                </h2>
                <span className="rounded-full bg-slate-900 px-2 py-0.5 text-[10px] font-bold text-white">
                  {filteredAuditLogs.length} {isAr ? 'حركة مسجلة' : 'Events'}
                </span>
              </div>
              <p className="text-xs text-slate-500 mt-1">
                {isAr
                  ? 'سجل غير قابل للتعديل يوثق كافة العمليات الحساسة (حذف الفواتير، تعديلات الأستاذ العام، تغيرات الحالة، الاعتمادات، والتجاوزات) مع توثيق هوية المستخدم وعنوان الـ IP.'
                  : 'Immutable regulatory log recording all sensitive ERP actions (invoice deletions, ledger edits, status changes, signatures, and overrides).'}
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button
                ...
                title={isAr ? 'تصدير السجل إلى ملف Excel/CSV' : 'Export Audit Log to CSV'}
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-colors shadow-2xs"
                title={isAr ? 'تصدير بصيغة JSON للامتثال والرقابة' : 'Export JSON Payload'}
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 rounded-xl border border-rose-200 bg-rose-50/60 px-3 py-1.5 text-xs font-bold text-rose-700 hover:bg-rose-100 transition-colors"
                title={isAr ? 'تفريغ السجلات (للإدارة العليا فقط)' : 'Clear audit log storage'}
              >
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div className="rounded-2xl border border-slate-200/70 bg-slate-50/60 p-3.5">
              <span className="text-[11px] font-bold text-slate-500 block">
                {isAr ? 'إجمالي الحركات الموثقة' : 'Total Logged Events'}
              </span>
              <div className="mt-1 flex items-center justify-between">
                <span className="text-xl font-black text-slate-900">{auditLogs.length}</span>
                <span className="rounded-lg bg-slate-200/80 px-2 py-0.5 text-[10px] font-bold text-slate-700">
                  {isAr ? '100% موثق' : 'Immutable'}
                </span>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-rose-100 bg-rose-50/40 p-3.5">
              <span className="text-[11px] font-bold text-rose-700 block">
                {isAr ? 'عمليات حساسة (حذف / تعديل أستاذ)' : 'Critical (Deletes/Edits)'}
              </span>
              <div className="mt-1 flex items-center justify-between">
                <span className="text-xl font-black text-rose-700">
                  {auditLogs.filter((l) => ['DELETE', 'OVERRIDE_DB', 'LEDGER_EDIT'].includes(l.action)).length}
                </span>
                <span className="rounded-lg bg-rose-100 px-2 py-0.5 text-[10px] font-bold text-rose-800">
                  {isAr ? 'تنبيه أمني' : 'High Audit'}
                </span>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-blue-100 bg-blue-50/40 p-3.5">
              <span className="text-[11px] font-bold text-blue-700 block">
                {isAr ? 'تغيرات الحالة والاعتمادات' : 'Status & Approvals'}
              </span>
              <div className="mt-1 flex items-center justify-between">
                <span className="text-xl font-black text-blue-700">
                  {auditLogs.filter((l) => ['STATUS_CHANGE', 'APPROVE', 'SIGN'].includes(l.action)).length}
                </span>
                <span className="rounded-lg bg-blue-100 px-2 py-0.5 text-[10px] font-bold text-blue-800">
                  {isAr ? 'مراحل العمل' : 'Transitions'}
                </span>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-emerald-100 bg-emerald-50/40 p-3.5">
              <span className="text-[11px] font-bold text-emerald-700 block">
                {isAr ? 'المستخدمون النشطون بالسجل' : 'Active Logged Users'}
              </span>
              <div className="mt-1 flex items-center justify-between">
          ...
          {/* Filtering Bar */}
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200/80 bg-slate-50/60 p-3">
            <div className="flex flex-wrap items-center gap-2 flex-1 min-w-[280px]">
              ...
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input
                  type="text"
                  placeholder={isAr ? 'بحث بالبيان، اسم المستخدم، رقم القيد/الفاتورة...' : 'Search logs, user, entity ID...'}
                  value={auditSearch}
                  onChange={(e) => setAuditSearch(e.target.value)}
                  className="w-full rounded-xl border border-slate-200 bg-white py-1.5 pl-3 pr-8 text-xs font-bold text-slate-900 focus:outline-none focus:border-orange-500"
                />
              </div>
              ...
              <select
                ...
                className="rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-800 focus:outline-none focus:border-orange-500"
              >
                <option value="ALL">{isAr ? 'كافة العمليات (All Actions)' : 'All Actions'}</option>
                <option value="STATUS_CHANGE">{isAr ? 'STATUS_CHANGE (تغيير الحالة)' : 'STATUS_CHANGE'}</option>
                <option value="LEDGER_EDIT">{isAr ? 'LEDGER_EDIT (تعديل الأستاذ العام)' : 'LEDGER_EDIT'}</option>
                <option value="JOURNAL_ENTRY">{isAr ? 'JOURNAL_ENTRY (قيد محاسبي)' : 'JOURNAL_ENTRY'}</option>
                <option value="DELETE">{isAr ? 'DELETE (حذف)' : 'DELETE'}</option>
                <option value="CREATE">{isAr ? 'CREATE (إنشاء)' : 'CREATE'}</option>
                <option value="UPDATE">{isAr ? 'UPDATE (تعديل)' : 'UPDATE'}</option>
                <option value="APPROVE">{isAr ? 'APPROVE (اعتماد)' : 'APPROVE'}</option>
                <option value="REJECT">{isAr ? 'REJECT (رفض)' : 'REJECT'}</option>
                <option value="SIGN">{isAr ? 'SIGN (توقيع رقمي)' : 'SIGN'}</option>
                <option value="OVERRIDE_DB">{isAr ? 'OVERRIDE (تجاوز مباشر)' : 'OVERRIDE_DB'}</option>
                <option value="SOFT_DELETE">{isAr ? 'SOFT_DELETE (أرشفة)' : 'SOFT_DELETE'}</option>
                <option value="RESTORE">{isAr ? 'RESTORE (استعادة)' : 'RESTORE'}</option>
              </select>
              ...
              >
                ...
                <option value="Invoice">{isAr ? 'فواتير المبيعات (Invoice)' : 'Invoice'}</option>
                <option value="JournalEntry">{isAr ? 'القيود المحاسبية (Journal Entry)' : 'Journal Entry'}</option>
                <option value="Voucher">{isAr ? 'السندات المالية (Voucher)' : 'Voucher'}</option>
                <option value="Operation">{isAr ? 'سجلات العمليات (Operation)' : 'Operation'}</option>
                <option value="Account">{isAr ? 'دليل الحسابات (Account)' : 'Account'}</option>
                <option value="Customer">{isAr ? 'العملاء (Customer)' : 'Customer'}</option>
                <option value="Crusher">{isAr ? 'الكسارات (Crusher)' : 'Crusher'}</option>
                <option value="Transporter">{isAr ? 'الناقلين (Transporter)' : 'Transporter'}</option>
                <option value="User">{isAr ? 'المستخدمين (User)' : 'User'}</option>
                <option value="BrandConfig">{isAr ? 'الهوية البصرية (Brand Config)' : 'Brand Config'}</option>
              </select>
            </div>

            {(auditSearch || auditActionFilter !== 'ALL' || auditEntityFilter !== 'ALL') && (
              <button
                type="button"
                onClick={() => {
                  setAuditSearch('');
                ...
                className="text-xs font-bold text-[#F05627] hover:underline"
              >
            ...
            {filteredAuditLogs.length === 0 ? (
              <div className="p-12 text-center text-slate-400 text-xs">
                ...
                <p className="font-bold text-slate-600">
                  {isAr ? 'لا توجد سجلات تدقيق تطابق معايير البحث الحالية' : 'No audit records match filters'}
                </p>
                <p className="text-[11px] text-slate-400 mt-1">
                  {isAr ? 'قم بتعديل الفلاتر أو إعادة ضبط البحث' : 'Adjust search keywords or action filters'}
                </p>
              </div>
            ) : (
              filteredAuditLogs.map((log) => {
                const isCritical = ['DELETE', 'OVERRIDE_DB', 'LEDGER_EDIT'].includes(log.action);
                const isStatusChange = ['STATUS_CHANGE', 'APPROVE', 'SIGN'].includes(log.action);
                const isCreate = ['CREATE', 'JOURNAL_ENTRY'].includes(log.action);
                  ...
                  >
                    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                      <div className="flex items-start gap-3">
                        {/* Action Badge */}
                        <span
                          className={`mt-0.5 rounded-lg px-2.5 py-1 text-[10px] font-mono font-black shrink-0 ${
                            log.action === 'CREATE' || log.action === 'JOURNAL_ENTRY'
                        ...
                        <div className="space-y-1">
                          <div className="flex flex-wrap items-center gap-2">
                            <p className="text-xs font-black text-slate-900">{log.summary}</p>
                            {log.entityType && (
                          ...
                          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-slate-500">
                            <span className="font-bold text-slate-700">
                              👤 {log.userName}{' '}
                              <span className="text-[10px] font-normal text-slate-400">({log.userRole})</span>
                            </span>
                            <span>•</span>
                            <span>⏱️ {formatDate(log.timestamp)}</span>
                            <span>•</span>
                      ...
                      {/* Right actions: Diff Toggle and Copy */}
                      <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
                        <button
                          ...
                          title={isAr ? 'نسخ بيانات الحركة JSON' : 'Copy JSON'}
                        >
                          {copiedLogId === log.id ? (
                            <>
                        ...
                        {(log.oldData || log.newData) && (
                          <button
                            ...
                            className="rounded-lg bg-orange-50 border border-orange-200/80 px-2.5 py-1 text-xs font-bold text-[#F05627] hover:bg-orange-100 transition-colors"
                          >
                    ...
                    {/* Expandable Visual Diff and Raw JSON */}
                    {expandedLogId === log.id && (
                          ...
                          <div className="rounded-xl border border-rose-200 bg-rose-50/50 p-3">
                            <div className="flex items-center justify-between mb-1.5 border-b border-rose-200/60 pb-1">
                              <span className="text-[11px] font-black text-rose-800">
                                {isAr ? 'الحالة السابقة (Before Change)' : 'Previous State (Old)'}
                              </span>
                              <span className="rounded bg-rose-100 px-1.5 py-0.5 text-[9px] font-bold text-rose-700">
                                OLD
                              </span>
                            </div>
                            <pre className="max-h-48 overflow-y-auto text-[11px] font-mono text-rose-950 whitespace-pre-wrap">
                              {log.oldData ? JSON.stringify(log.oldData, null, 2) : isAr ? '(لا توجد بيانات سابقة - عملية إنشاء جديدة)' : '(None - Initial Creation)'}
                            </pre>
                          </div>
                          ...
                          <div className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-3">
                            <div className="flex items-center justify-between mb-1.5 border-b border-emerald-200/60 pb-1">
                              <span className="text-[11px] font-black text-emerald-800">
                                {isAr ? 'الحالة الحالية والجديدة (After Change)' : 'Applied State (New)'}
                              </span>
                              <span className="rounded bg-emerald-100 px-1.5 py-0.5 text-[9px] font-bold text-emerald-700">
                                NEW
                              </span>
                            </div>
                            <pre className="max-h-48 overflow-y-auto text-[11px] font-mono text-emerald-950 whitespace-pre-wrap">
                              {log.newData ? JSON.stringify(log.newData, null, 2) : isAr ? '(تم الحذف بالكامل)' : '(Deleted)'}
                            </pre>
                          </div>
                        </div>
                      </div>
