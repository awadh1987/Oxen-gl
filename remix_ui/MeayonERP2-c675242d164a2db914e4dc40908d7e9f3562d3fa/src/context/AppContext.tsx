...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [currentUser, setCurrentUser] = useState<User>(() => {
    if (!localStorage.getItem('meayon_access_token')) {
  ...
  useEffect(() => {
    if (currentUser.role !== 'Admin' || !localStorage.getItem('meayon_access_token')) return;

    void fetch('/api/user-registrations', {
      headers: { Authorization: `Bearer ${localStorage.getItem('meayon_access_token')}` },
    }).then(async (response) => {
  ...
  useEffect(() => {
    if (!localStorage.getItem('meayon_access_token')) return;

    void apiService.getWorkflowRecords().then((records) => {
      if (records.operations?.length) {
        setOperations(records.operations.map((record: any) => ({
          id: record.id,
      ...
      if (records.vouchers?.length) {
        setVouchers(records.vouchers.map((record: any) => ({
          id: record.id,
      ...
      if (records.journalEntries?.length) {
        setJournalEntries(records.journalEntries.map((record: any) => ({
          id: record.id,
          date: record.date,
  ...
  // Firebase Authentication & Sync State
    ...
    try {
      const unsubscribe = onAuthStateChanged(auth, (fUser) => {
        ...
        if (fUser) {
          setCurrentUser((prev) => ({
            ...prev,
            fullName: fUser.displayName || prev.fullName,
      ...
      console.warn('Firebase Auth state listener notice:', e);
    }
  }, []);
    ...
    try {
      ...
      if (res.user) {
        ...
        try {
          ...
          console.warn('Could not get idToken, using client user credentials:', tokErr);
        }

        // Call /api/auth/verify to resolve user role, seed binding, and authorization status
        let verifiedUserData: Partial<User> = {};
        try {
          ...
          console.warn('Auth verify endpoint fallback:', apiErr);
        }

        const existing = users.find((u) => u.email === res.user.email);
        const signedInUser: User = {
          id: verifiedUserData.id || existing?.id || res.user.uid,
          username: existing?.username || res.user.email?.split('@')[0] || 'google_user',
          fullName: verifiedUserData.fullName || existing?.fullName || res.user.displayName || 'Google User',
          fullNameAr: verifiedUserData.fullNameAr || existing?.fullNameAr || res.user.displayName || 'مستخدم جوجل المعتمد',
          role: verifiedUserData.role || existing?.role || 'Admin',
          email: res.user.email || '',
          firebaseUid: res.user.uid,
          avatar: res.user.photoURL || existing?.avatar || undefined,
          status: (verifiedUserData.status as any) || existing?.status || 'Active',
        };

        // If not in users list, save to users list
        setUsers((prev) => {
          const idx = prev.findIndex((u) => u.email === signedInUser.email);
          if (idx >= 0) {
            const copy = [...prev];
            copy[idx] = { ...copy[idx], ...signedInUser };
            return copy;
          }
          return [...prev, signedInUser];
        });

        setCurrentUser(signedInUser);
        logAuditAction({
          userId: signedInUser.id,
          userName: signedInUser.fullNameAr || signedInUser.fullName,
          userRole: signedInUser.role,
          action: 'LOGIN',
          entityType: 'User',
          entityId: signedInUser.id,
          summary: `تسجيل الدخول والتوثيق الناجح عبر حساب Google المعتمد (${res.user.email})`,
        });
        return signedInUser;
      }
      return null;
  ...
  const approveUserRequest = async (id: string, assignedRole: UserRole) => {
    ...
    const response = await fetch(`/api/user-registrations/${id}/approve`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('meayon_access_token') || ''}` },
